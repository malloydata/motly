...
describe("Mot", () => {
  ...
  describe("typed accessors", () => {
    ...
    it("numeric returns numeric value", () => {
      assert.equal(mot("x = 8080").get("x").numeric(), 8080);
    });

    it("numeric returns undefined for string", () => {
      assert.equal(mot("x = hello").get("x").numeric(), undefined);
    });
    ...
    it("all accessors return undefined on Undefined Mot", () => {
      ...
      assert.equal(u.numeric(), undefined);
      ...
      assert.equal(u.numerics(), undefined);
  ...
  describe("navigation", () => {
    it("get() walks nested properties", () => {
      ...
      assert.equal(m.get("server", "port").numeric(), 3000);
    });
    ...
    it("get() with numeric index navigates array elements", () => {
      const m = mot("items = [a, b, c]");
      assert.equal(m.get("items", 0).text(), "a");
      assert.equal(m.get("items", 1).text(), "b");
      assert.equal(m.get("items", 2).text(), "c");
    });

    it("get() with numeric index out of bounds returns Undefined Mot", () => {
      const m = mot("items = [a, b]");
      assert.equal(m.get("items", 5).exists, false);
    });

    it("get() with numeric index on non-array returns Undefined Mot", () => {
      const m = mot("x = hello");
      assert.equal(m.get("x", 0).exists, false);
    });

    it("get() with non-integer numeric index returns Undefined Mot", () => {
      const m = mot("items = [a, b, c]");
      assert.equal(m.get("items", 1.5).exists, false);
      assert.equal(m.get("items", NaN).exists, false);
      assert.equal(m.get("items", Infinity).exists, false);
    });

    it("get() mixes string and numeric segments", () => {
      const m = mot("items = [one { x = 1 }, two { x = 2 }]");
      assert.equal(m.get("items", 0, "x").numeric(), 1);
      assert.equal(m.get("items", 1, "x").numeric(), 2);
    });
  });

  describe("pathed accessors", () => {
    it("text() with path navigates then accesses", () => {
      const m = mot("server { host = localhost }");
      assert.equal(m.text("server", "host"), "localhost");
    });

    it("numeric() with path", () => {
      const m = mot("server { port = 3000 }");
      assert.equal(m.numeric("server", "port"), 3000);
    });

    it("boolean() with path", () => {
      const m = mot("server { ssl = @true }");
      assert.equal(m.boolean("server", "ssl"), true);
    });

    it("valueType() with path", () => {
      const m = mot("server { port = 3000 }");
      assert.equal(m.valueType("server", "port"), "number");
    });

    it("pathed accessor on missing path returns undefined", () => {
      const m = mot("x = 1");
      assert.equal(m.text("nope", "deep"), undefined);
    });

    it("pathed accessor with numeric index", () => {
      const m = mot("items = [hello, world]");
      assert.equal(m.text("items", 0), "hello");
      assert.equal(m.text("items", 1), "world");
    });
  });
  ...
  describe("arrays", () => {
    ...
    it("array elements with properties", () => {
      ...
      assert.equal(vals[0].get("x").numeric(), 1);
    });
    ...
    it("numerics convenience accessor", () => {
      const n = mot("ports = [80, 443, 8080]").get("ports").numerics();
    ...
    it("texts works when elements have properties", () => {
      ...
      assert.equal(vals[0].get("p").numeric(), 1);
      assert.equal(vals[1].get("q").numeric(), 2);
    });

    it("numerics works when elements have properties", () => {
      const m = mot("x = [10 { unit = ms }, 20 { unit = s }]");
      assert.deepStrictEqual(m.get("x").numerics(), [10, 20]);
    });
    ...
    it("numerics returns undefined for mixed types", () => {
      assert.equal(mot("x = [1, hello]").get("x").numerics(), undefined);
    });
    ...
    it("empty array returns empty convenience arrays", () => {
      ...
      assert.deepStrictEqual(m.get("x").numerics(), []);
    ...
    it("nested arrays: outer values are arrays, not scalars", () => {
      ...
      assert.deepStrictEqual(outer[0].numerics(), [1, 2]);
      assert.deepStrictEqual(outer[1].numerics(), [3, 4]);
      // outer convenience accessors don't work (elements are arrays, not scalars)
      assert.equal(m.get("x").numerics(), undefined);
      assert.equal(m.get("x").texts(), undefined);
    });
  });
  ...
  describe("references", () => {
    ...
    it("resolves chained $ref", () => {
      const m = mot("a { x = 1 }\nb = $a\nc = $b");
      assert.equal(m.get("c", "x").numeric(), 1);
    });
    ...
    it("circular refs with a backing node share Mot instances", () => {
      const m = mot("a { x = 1 }\na.ref = $b\nb = $a");
      assert.equal(m.get("a", "x").numeric(), 1);
      assert.equal(m.get("b", "x").numeric(), 1);
      // b → $a → concrete node with x=1, ref pointing back to b
      assert.equal(m.get("b", "ref", "x").numeric(), 1);
    });
  ...
  describe("deletions", () => {
    it("deleted nodes do not appear", () => {
      const m = mot("a = 1\nb = 2\n-b");
      assert.equal(m.get("a").numeric(), 1);
      assert.equal(m.get("b").exists, false);

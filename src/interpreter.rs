...
fn resolve_array_element(
    ...
    if let Some(ref prop_stmts) = el.properties {
        ...
        let transformers = flatten(prop_stmts, ctx);
        let ChunkResult { splits, deps } = chunk(&transformers);
        let TopoSortResult { order, .. } = topo_sort(&deps);
        errors.extend(execute_chunked(&transformers, &splits, &order, &mut node, &ctx.options));
    }
